...
class Solution3:
    def isSameTree(self, p, q):
        ...
        def check(fnode, snode):
            if not fnode and not snode:
                return True
            if not fnode or not snode:
                return False
        ...
        while node_pair_queue:
            head_pair = node_pair_queue.pop(0)
            if not check(head_pair[0], head_pair[1]):
                return False
            if head_pair[0]:
                node_pair_queue.append((head_pair[0].left, head_pair[1].left))
                node_pair_queue.append((head_pair[0].right, head_pair[1].right))
        return True
